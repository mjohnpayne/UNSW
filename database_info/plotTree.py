...
def parse_args():
    ...
    parser.add_argument('--info', type=str, required=True,
                        help='data file (column 1 matches leaf names, other columns contain data for labelling)')
    parser.add_argument('--info_format', type=str, required=False, help='data file format: csv (default) or tab',
                        default='csv')
    parser.add_argument('--labels', nargs='+', type=str, required=False,
                        help='labels to use (must match column headers in the info file)')
    ...
    parser.add_argument('--colour_backgrounds_by', type=str, required=False,
                        help='variable to use for colouring clade backgrounds')
    ...
    parser.add_argument('--tags', action="store_true", required=False,
                        help='Colour label backgrounds to indicate values')
    parser.add_argument('--colour_tags', nargs='+', type=str, required=False,
                        help='labels to use to tag each element by colour code')
    ...
    parser.add_argument('--output', type=str, required=False,
                        help='output file (pdf or png, type is taken from the extension, eg out.pdf)')
    ...
    parser.add_argument('--padding', type=int, required=False,
                        help='padding between label columns (pixels, default 20)', default=20)
    parser.add_argument('--length_scale', type=int, required=False, help='scale (pixels per branch length unit)')
    parser.add_argument('--branch_padding', type=int, required=False,
                        help='branch (pixels between each branch, ie vertical padding)')
    parser.add_argument('--show_leaf_names', action="store_true", required=False,
                        help='Print leaf names as well as labels')
    parser.add_argument('--branch_support_print', action="store_true", required=False, help='Print branch supports')
    parser.add_argument('--branch_support_colour', action="store_true", required=False,
                        help='Colour branch leading to node by branch supports')
    parser.add_argument('--branch_support_percent', action="store_true", required=False,
                        help='Branch supports expressed as percent (0-100), otherwise assume scale is 0-1')
    parser.add_argument('--branch_support_cutoff', type=int, required=False,
                        help='Colour branches with support lower than this value')
    parser.add_argument('--no_guiding_lines', action="store_true", required=False,
                        help='Turn off linking nodes to data with guiding lines')
    parser.add_argument('--fan', action="store_true", required=False, help='Plot tree as fan dendrogram')
    parser.add_argument('--colour_dict', type=str, required=False,
                        help='manually specify dictionary of values -> colours')
    parser.add_argument('--print_colour_dict', action="store_true", required=False,
                        help='Print colour dictionary for later use')
    parser.add_argument('--title', type=str, required=False, help='Title for plot')
    parser.add_argument('--data', type=str, required=False,
                        help='Data matrix (tab delmited; header row starts with "#Names col1 col2 etc...", column 1 matches leaf names, other columns should be numerical values for plotting)')
    parser.add_argument('--data_type', type=str, required=False,
                        help='Type of data plot ([heatmap], line_profiles, bars, cbars)', default="heatmap")
    parser.add_argument('--data_width', type=int, required=False,
                        help='Total width of data plot for each strain (mm, default 200)', default=200)
    parser.add_argument('--data_height', type=int, required=False,
                        help='Total height of data plot for each strain (mm, default 20)', default=20)
    parser.add_argument('--mindata', type=int, required=False, help='Minimum data value for plotting scale (-1)',
                        default=-1)
    parser.add_argument('--maxdata', type=int, required=False, help='Maximum data value for plotting scale (1)',
                        default=1)
    parser.add_argument('--centervalue', type=int, required=False, help='Central data value for plotting scale (0)',
                        default=0)
    ...
    parser.add_argument('--interactive', action="store_true", required=False,
                        help='Switch on interactive view (after printing tree to file)')
...
def readtable(args, leaves):
    ...
    table = collections.defaultdict(
        dict)  # key 1 = row identifier (column 1), key 2 = column identifier, value = cell entry
    f = file(args.info, "r")
    column_list = []
    ...
    for line in f:
        ...
        if len(column_list) == 0:
            ...
            if len(column_list) != len(fields):
                print "\nWARNING: " + str(len(column_list)) + " column headings, " + str(
                    len(fields)) + " entries in row " + row_id
                print "Using delimiter " + args.info_format + ", is this correct?"
            if row_id in leaves:
...
def getColourPalette(values, args, this_label):
    ...
    colours_50 = ["#E41A1C", "#C72A35", "#AB3A4E", "#8F4A68", "#735B81", "#566B9B", "#3A7BB4", "#3A85A8", "#3D8D96",
                  "#419584", "#449D72", "#48A460", "#4CAD4E", "#56A354", "#629363", "#6E8371", "#7A7380", "#87638F",
                  "#93539D", "#A25392", "#B35A77", "#C4625D", "#D46A42", "#E57227", "#F67A0D", "#FF8904", "#FF9E0C",
                  "#FFB314", "#FFC81D", "#FFDD25", "#FFF12D", "#F9F432", "#EBD930", "#DCBD2E", "#CDA12C", "#BF862B",
                  "#B06A29", "#A9572E", "#B65E46", "#C3655F", "#D06C78", "#DE7390", "#EB7AA9", "#F581BE", "#E585B8",
                  "#D689B1", "#C78DAB", "#B791A5", "#A8959F", "#999999"]
    simple_palette = ["Red", "DarkBlue", "Gold", "LimeGreen", "Violet", "MediumTurquoise", "Sienna", "LightCoral",
                      "LightSkyBlue", "Indigo", "Tan", "Coral", "OliveDrab", "Teal"]
...
def main():
    ...
    if args.colour_branches_by or args.colour_backgrounds_by or args.branch_support_colour:
        ...
        if args.colour_backgrounds_by:
            ...
            colour_dict_bg = getColourPalette(column_values[args.colour_backgrounds_by], args,
        ...
        if args.branch_support_colour:
            ...
            if args.branch_support_cutoff:
                ...
                if args.branch_support_percent:
                    support_colours = {0: "#FF0000", 1: "#FF0000", 2: "#FC0000", 3: "#F90000", 4: "#F70000",
                                       5: "#F40000", 6: "#F20000", 7: "#EF0000", 8: "#EC0000", 9: "#EA0000",
                                       10: "#E70000", 11: "#E50000", 12: "#E20000", 13: "#E00000", 14: "#DD0000",
                                       15: "#DA0000", 16: "#D80000", 17: "#D50000", 18: "#D30000", 19: "#D00000",
                                       20: "#CE0000", 21: "#CB0000", 22: "#C80000", 23: "#C60000", 24: "#C30000",
                                       25: "#C10000", 26: "#BE0000", 27: "#BC0000", 28: "#B90000", 29: "#B60000",
                                       30: "#B40000", 31: "#B10000", 32: "#AF0000", 33: "#AC0000", 34: "#AA0000",
                                       35: "#A70000", 36: "#A40000", 37: "#A20000", 38: "#9F0000", 39: "#9D0000",
                                       40: "#9A0000", 41: "#970000", 42: "#950000", 43: "#920000", 44: "#900000",
                                       45: "#8D0000", 46: "#8B0000", 47: "#880000", 48: "#850000", 49: "#830000",
                                       50: "#800000", 51: "#7E0000", 52: "#7B0000", 53: "#790000", 54: "#760000",
                                       55: "#730000", 56: "#710000", 57: "#6E0000", 58: "#6C0000", 59: "#690000",
                                       60: "#670000", 61: "#640000", 62: "#610000", 63: "#5F0000", 64: "#5C0000",
                                       65: "#5A0000", 66: "#570000", 67: "#540000", 68: "#520000", 69: "#4F0000",
                                       70: "#4D0000", 71: "#4A0000", 72: "#480000", 73: "#450000", 74: "#420000",
                                       75: "#400000", 76: "#3D0000", 77: "#3B0000", 78: "#380000", 79: "#360000",
                                       80: "#330000", 81: "#300000", 82: "#2E0000", 83: "#2B0000", 84: "#290000",
                                       85: "#260000", 86: "#240000", 87: "#210000", 88: "#1E0000", 89: "#1C0000",
                                       90: "#190000", 91: "#170000", 92: "#140000", 93: "#120000", 94: "#0F0000",
                                       95: "#0C0000", 96: "#0A0000", 97: "#070000", 98: "#050000", 99: "#020000",
                                       100: "#000000"}
                else:
                    support_colours = {0.0: "#FF0000", 0.01: "#FF0000", 0.02: "#FC0000", 0.03: "#F90000",
                                       0.04: "#F70000", 0.05: "#F40000", 0.06: "#F20000", 0.07: "#EF0000",
                                       0.08: "#EC0000", 0.09: "#EA0000", 0.1: "#E70000", 0.11: "#E50000",
                                       0.12: "#E20000", 0.13: "#E00000", 0.14: "#DD0000", 0.15: "#DA0000",
                                       0.16: "#D80000", 0.17: "#D50000", 0.18: "#D30000", 0.19: "#D00000",
                                       0.2: "#CE0000", 0.21: "#CB0000", 0.22: "#C80000", 0.23: "#C60000",
                                       0.24: "#C30000", 0.25: "#C10000", 0.26: "#BE0000", 0.27: "#BC0000",
                                       0.28: "#B90000", 0.29: "#B60000", 0.3: "#B40000", 0.31: "#B10000",
                                       0.32: "#AF0000", 0.33: "#AC0000", 0.34: "#AA0000", 0.35: "#A70000",
                                       0.36: "#A40000", 0.37: "#A20000", 0.38: "#9F0000", 0.39: "#9D0000",
                                       0.4: "#9A0000", 0.41: "#970000", 0.42: "#950000", 0.43: "#920000",
                                       0.44: "#900000", 0.45: "#8D0000", 0.46: "#8B0000", 0.47: "#880000",
                                       0.48: "#850000", 0.49: "#830000", 0.5: "#800000", 0.51: "#7E0000",
                                       0.52: "#7B0000", 0.53: "#790000", 0.54: "#760000", 0.55: "#730000",
                                       0.56: "#710000", 0.57: "#6E0000", 0.58: "#6C0000", 0.59: "#690000",
                                       0.6: "#670000", 0.61: "#640000", 0.62: "#610000", 0.63: "#5F0000",
                                       0.64: "#5C0000", 0.65: "#5A0000", 0.66: "#570000", 0.67: "#540000",
                                       0.68: "#520000", 0.69: "#4F0000", 0.7: "#4D0000", 0.71: "#4A0000",
                                       0.72: "#480000", 0.73: "#450000", 0.74: "#420000", 0.75: "#400000",
                                       0.76: "#3D0000", 0.77: "#3B0000", 0.78: "#380000", 0.79: "#360000",
                                       0.8: "#330000", 0.81: "#300000", 0.82: "#2E0000", 0.83: "#2B0000",
                                       0.84: "#290000", 0.85: "#260000", 0.86: "#240000", 0.87: "#210000",
                                       0.88: "#1E0000", 0.89: "#1C0000", 0.9: "#190000", 0.91: "#170000",
                                       0.92: "#140000", 0.93: "#120000", 0.94: "#0F0000", 0.95: "#0C0000",
                                       0.96: "#0A0000", 0.97: "#070000", 0.98: "#050000", 0.99: "#020000",
                                       1.0: "#000000"}
        for node in tree.traverse():
            ...
            if node.name in table:
                ...
                if args.colour_branches_by:
                    nstyle["vt_line_color"] = colour_dict_br[
                        table[node.name][args.colour_branches_by]]  # set branch colour
                ...
                if args.colour_backgrounds_by:
                    if args.colour_branches_by in table[node.name]:
                        if table[node.name][args.colour_branches_by] != "none":
                            nstyle["bgcolor"] = colour_dict_bg[
                                table[node.name][args.colour_backgrounds_by]]  # set background colour
    ...
    if args.data:
        print "\nPrinting data matrix as " + args.data_type + " with range (" + str(args.mindata) + "->" + str(
            args.maxdata) + ";" + str(args.centervalue) + "), height " + str(args.data_height) + ", width " + str(
            args.data_width)
        profileFace = ProfileFace(min_v=args.mindata, max_v=args.maxdata, center_v=args.centervalue,
                                  width=args.data_width, height=args.data_height, style=args.data_type)
