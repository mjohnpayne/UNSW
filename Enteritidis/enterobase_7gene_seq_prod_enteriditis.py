...
strain_list = ["enteritidis"]

outannot = open("/Users/mjohnpayne/Documents/UNSW/Salmonella/enteriditis/7_gene_enteriditis_subset.txt","w")
...
for i in indata[1:]:
    col = i.strip('\n').split("\t")
    sv = str.lower(col[19])
    if col[11] != "" and sv != "" and is_number(sv[0]) == False and col[34] != "NaN":
        if int(col[34]) > 0:
            sv = sv.split(" ")[0]
