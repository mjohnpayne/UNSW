...
def parse_snps(snps):
    ...
    outd = {x[1] : [x[3], x[4]] for x in inf if x[2]=="snp" and len(x) > 2}
    return outd
...
def determine_or_assign_st(new,st,profile,inaccs):
    ...
    outdict = {x:[] for x in schemels}
    for i in schemels:
        if new:
            inf = open(inaccs + i + "_gene_accessions.txt", "r").read().strip("\n").splitlines()
            scheme_profiles = pd.read_csv(prefix + "_hierarchical_ST/"+i+"_gene_profiles.txt", sep="\t", header=0)
    ...
    for i in schemels:
        ...
        outf.write(str(outdict["stmcgMLST"]) + "\t" + stype + "\n")
        outf.close()
    outstring.write('\n')
    outstring.close()
...
def main(profile,al_loc,snpdb,snp,genome,annot,STM=False):
    if check_strain(strain,annot)==True:
        ...
        if val == True:
            ...
            # determine_or_assign_st(False,ids,"",loci_lists)
            inf = open(hier_assign,"r").read().strip('\n').splitlines()
            inf = {x.split('\t')[-1]:x.split('\t')[1:] for x in inf}
            idls = [x.split('\t')[-1] for x in inf]
            if ids in idls:
                t = inf[ids]
                outf = open(hier_assign, "a+")
                outf.write(strain + "\t" + "\t".join(t) +'\n')
                outf.close()
            annotations.close()
        # schemels = ["7","20","50","100","250","500","750","1000","cgMLST"]
